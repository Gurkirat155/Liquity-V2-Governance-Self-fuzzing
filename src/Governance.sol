...
contract Governance is MultiDelegateCall, UserProxyFactory, ReentrancyGuard, Ownable, IGovernance {
    ...
    /// @inheritdoc IGovernance
    // @latest @audit not understanding this
    uint256 public immutable MIN_CLAIM;
    /// @inheritdoc IGovernance
    // how much bold tokens were avaialble for the the initiative
    // @latest @audit not understanding this
    uint256 public immutable MIN_ACCRUAL; // this is 1000e18
    /// @inheritdoc IGovernance
    uint256 public immutable REGISTRATION_FEE;
    /// @inheritdoc IGovernance
    // @audit gather inforamtion arounf the below state variables
    // this is minimum number of voting power or offset required for an user who is making an initiave to get registererd
    uint256 public immutable REGISTRATION_THRESHOLD_FACTOR;
    /// @inheritdoc IGovernance
    // this is minimum number of voting power or offset required in vetos for a initiave to unregister
    ...
    //     uint256 lastEpochClaim;     // @audit what is this.
    // }
    ...
    function withdrawLQTY(uint256 _lqtyAmount, bool _doSendRewards, address _recipient) public nonReentrant {
        ...
        if (_lqtyAmount < userState.unallocatedLQTY) {
            ...
            // @audit is this decrease in the offsetDecrease in the voting power
    ...
    /// @inheritdoc IGovernance
    //   User voting power at time t:
    // _lqtyAmount = user’s total unallocated LQTY
    // _offset = user’s unallocated offset
    // This gives how much voting power the user has available right now.
    // This is a very gas-efficient way of tracking time-based voting power without 
    // needing to store deposit timestamps for every stake.
    ...
    */
            ...
            for (uint256 x; x < _initiatives.length; x++) {
                ...
                for (uint256 y; y < cachedData.length; y++) {
                    if (cachedData[y].initiative == _initiatives[x]) {
                        found = true;
                        // @audit need the below line to understand it's meaning
    ...
    function _allocateLQTY(
        ...
        for (uint256 i = 0; i < _initiatives.length; i++) {
            ...
            // @audit still having doubts why is the below one needed 
